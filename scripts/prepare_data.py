...
def prepare_slimpajama(output, limit, plan_only, caps):
 from huggingface_hub import HfApi,hf_hub_download
 api=HfApi(); info=api.dataset_info("DKYoon/SlimPajama-6B"); revision=info.sha
...
def main():
 p=argparse.ArgumentParser();p.add_argument("--kind",choices=["synthetic","slimpajama"],required=True);p.add_argument("--output",required=True);p.add_argument("--plan-only",action="store_true");p.add_argument("--source-max-bytes",type=int,default=536870912);p.add_argument("--seed",type=int,default=1337);p.add_argument("--train-tokens",type=int,default=131073);p.add_argument("--validation-tokens",type=int,default=8193);p.add_argument("--test-tokens",type=int,default=8193);p.add_argument("--overfit",action="store_true");a=p.parse_args();output=Path(a.output)
 if a.kind=="synthetic":
  ...
  prepare_slimpajama(output,a.source_max_bytes,a.plan_only,counts)
