...
def run(cfg: dict, run_dir: Path, resume: Path | None = None, max_wall_seconds: float | None = None, to_device: str = "auto"):
    ...
    if completed == 0:
        ev = evaluate(model, val, cfg["eval"]["max_target_tokens"], cfg["eval"]["batch_size"], cfg, device); writer.write("eval", 0, 0, split=cfg["data"]["validation_split"], **ev)
    ...
    try:
        while completed < cfg["derived"]["total_updates"]:
            if max_wall_seconds is not None and time.monotonic()-started >= max_wall_seconds: status, reason = "paused_budget", "max_wall_seconds reached at update boundary"; break
            ...
            writer.write("train",completed,processed,lr=lr,train_nll=nll_sum/n_tokens,pre_clip_grad_norm=float(grad_norm),clipped=clipped,elapsed_seconds=time.monotonic()-step_started,cuda_peak_allocated=torch.cuda.max_memory_allocated() if device.type=="cuda" else None,cuda_peak_reserved=torch.cuda.max_memory_reserved() if device.type=="cuda" else None)
            do_eval = cfg["eval"]["every_updates"] > 0 and completed % cfg["eval"]["every_updates"] == 0
            if do_eval or completed == cfg["derived"]["total_updates"]:
                ev=evaluate(model,val,cfg["eval"]["max_target_tokens"],cfg["eval"]["batch_size"],cfg,device); writer.write("eval",completed,processed,split=cfg["data"]["validation_split"],**ev)
            if completed % cfg["checkpoint"]["every_updates"] == 0 or completed == cfg["derived"]["total_updates"]:
                ck=_checkpoint(cfg,model,optimizer,sampler,completed,processed,run_id,segment,prior_elapsed+time.monotonic()-started,manifest,records); atomic_torch_save(ck,run_dir/"checkpoints/latest.pt")
    ...
    summary={"status":status,"reason":reason,"completed_updates":completed,"processed_target_tokens":processed,"total_updates":cfg["derived"]["total_updates"],"elapsed_seconds":elapsed,"optimizer_state":state}; write_json(run_dir/"summary.json",summary); return summary
