...
def load_recipe(path: str | Path) -> dict[str, Any]:
    ...
    simulations = {"none", "bf16_roundtrip", "int8_linear_first_moment", "int8_linear_second_moment", "int8_linear_all_moments", "int8_linear_momentum", "int4_linear_momentum", "int8_dynamic_all_moments", "int8_dynamic_second_moment", "int4_dynamic_all_moments"}
    ...
    if o["name"] == "reference_adamw" and o["state_simulation"] in {"int8_linear_momentum", "int4_linear_momentum"}:
        raise RecipeError("linear momentum simulations are only valid for reference_muon")
    ...
    if granularity == "blockwise" and not (o["state_simulation"].startswith(("int8_linear_", "int4_linear_")) or o["state_simulation"] in {"int8_dynamic_all_moments", "int8_dynamic_second_moment", "int4_dynamic_all_moments"}):
        raise RecipeError("blockwise state quantization requires an integer state simulation")
    if o["state_simulation"] in {"int8_dynamic_all_moments", "int8_dynamic_second_moment", "int4_dynamic_all_moments"} and granularity != "blockwise":
